...
st.sidebar.write(" Çalışan dosya:", __file__)
st.sidebar.write("Lang:", st.session_state.get("lang", "—"))
st.write(" appnew.py boot OK")
...
USERS = {
    ...
    "personel1": {"password": "abcd", "role": "personel"}
}
...
if not st.session_state.logged_in:
    ...
    if st.session_state.lang == "Türkçe":
        ...
        if st.button("Giriş Yap", key="__login_btn__"):
        ...
        if st.button("Login", key="__login_btn__"):
            if username in USERS and USERS[username]["password"] == password:
                st.session_state.logged_in = True
                st.session_state.role = USERS[username]["role"]
                st.session_state.user = username
